...
public:
	...
	bool operator<(const CharString &compare) const;//
	bool operator==(const CharString &compare) const;//
	friend CharString operator+(const CharString &plus, const CharString &addplus); //字符串的连接
	friend std::istream& operator>>(std::istream& in, CharString& CharString); //重载了输入流
	friend std::ostream& operator<<(std::ostream& out, CharString& CharString); //重载输出流
	friend ifstream &operator >> (ifstream &fin, CharString &yc); //重载输入流
